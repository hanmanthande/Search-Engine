...
from app.models import OCRDocument, OCRChunk, User
from app.schemas import (
    UploadResponse, SearchResponse, SearchResult,
    AskResponse, DocumentListResponse, DocumentInfo,
)
...
def _get_nearby_context(db, document_id, chunk_number, window=1):
    nearby = (
        db.query(OCRChunk)
        .filter(
...
def _user_docs(db, user):
    return db.query(OCRDocument).filter(OCRDocument.owner_id == user.id)
...
async def upload_document(
    ...
    try:
        if ext == ".pdf":
            ...
            for page_num, page_text in enumerate(pages, start=1):
                for chunk_num, chunk in enumerate(split_text(page_text), start=1):
                    if not chunk.strip():
                        continue
                    db.add(OCRChunk(
                        document_id=document.id, page_number=page_num,
                        chunk_number=chunk_num, content=chunk,
    ...
    return UploadResponse(
        document_id=document.id, filename=file.filename, s3_key=s3_key,
        file_size=len(file_bytes), page_count=document.page_count or 1,
...
def keyword_search(
    ...
    matches = (
        db.query(OCRChunk).join(OCRDocument)
        .filter(OCRDocument.owner_id == current_user.id, OCRChunk.content.ilike(f"%{query}%"))
    ...
    for chunk in matches:
        ...
        results.append(SearchResult(
            ...
            filename=chunk.document.filename, s3_key=chunk.document.s3_key,
            page=chunk.page_number, chunk_number=chunk.chunk_number,
...
def semantic_search(
    ...
    try:
        raw_results = retrieve_context_with_metadata(query_embedding, db, top_k=limit, owner_id=current_user.id)
    except RuntimeError as e:
        raise HTTPException(500, str(e))
    ...
    for row in raw_results:
        ...
        results.append(SearchResult(
            ...
            page=row["page"], chunk_number=row["chunk_number"],
...
def ask_question(
    ...
    try:
        context_chunks = retrieve_context(query_embedding, db, top_k=top_k, owner_id=current_user.id)
    except RuntimeError as e:
        raise HTTPException(500, str(e))
...
def list_documents(
    ...
    base_q = _user_docs(db, current_user)
    total = base_q.with_entities(func.count(OCRDocument.id)).scalar()
    documents = base_q.order_by(OCRDocument.created_at.desc()).offset(offset).limit(limit).all()

    doc_infos = []
    for doc in documents:
        chunk_count = db.query(func.count(OCRChunk.id)).filter(OCRChunk.document_id == doc.id).scalar()
        doc_infos.append(DocumentInfo(
            id=doc.id, filename=doc.filename, s3_key=doc.s3_key,
...
def get_document_url(
    ...
    doc = _user_docs(db, current_user).filter(OCRDocument.id == document_id).first()
    if not doc:
