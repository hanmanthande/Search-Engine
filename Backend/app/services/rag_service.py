...
def retrieve_context(
    ...
    """
    Retrieves the top-k most semantically similar chunks from pgvector.
    Returns a list of content strings to be used as RAG context.
    """
    sql = text("""
        SELECT content
        FROM ocr_chunks
...
def retrieve_context_with_metadata(
    ...
    """
    Like retrieve_context, but also returns filename, page, chunk_number, and distance.
    Used by the /search/semantic endpoint.
    """
    sql = text("""
        SELECT
            c.id,
